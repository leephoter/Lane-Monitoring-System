...
for line in lines:
    rho, theta = line[0]
# Angle of Lines
    if(theta < np.pi*50/180 and theta > np.pi*43/180):
        thetaQ[0] = thetaQ[0] + theta
        rhoQ[0] = rhoQ[0] + rho
        countQ[0] = countQ[0] + 1
    if(theta < np.pi*140/180 and theta > np.pi*138/180):
