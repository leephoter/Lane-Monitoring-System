...
# Canny
edges = cv.Canny(imgGray, 50, 150)  # changing inputs
...
# Hough Line
lines = cv.HoughLines(edges, 1, np.pi/200, 21)
...
cv.rectangle(imgHough, tuple(pt1[0]), tuple(pt2[0]), (0, 128, 256), 2)
cv.rectangle(imgHough, tuple(pt1[1]), tuple(pt2[1]), (128, 0, 0), 2)
myShow(fileName+'_Hough2', imgHough)

# Displaying the lane
point = np.array([tuple(pt2[0]),  tuple(pt1[0]), tuple(pt2[1]), tuple(pt1[1])], np.int32)
